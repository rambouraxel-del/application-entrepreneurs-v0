...
- **Pages secondaires fictives** :
  ...
  - **Produits / Services** *(catalogue depuis la V0.5.1, fiche complète depuis la V0.5.2)* : recherche et filtres par type et par statut **réellement fonctionnels**, compteur dynamique du nombre d'éléments affichés, tableau de 8 produits/services fictifs (Nom, Type, Prix HT, TVA par défaut, Statut), chaque ligne disposant d'un bouton "Voir la fiche" vers `pages/fiche-produit-service.html` ;
  - **Fiche produit/service** *(complète depuis la V0.5.2)* : en-tête (nom, type, statut, description, prix HT, TVA), résumé commercial (5 indicateurs dont Coût estimé et Marge estimée en placeholders statiques), description commerciale (description détaillée, bénéfices client, limites/conditions), paramètres de vente (prix modifiable, conditions particulières, modalités de paiement en badges), bloc "Utilisation future en facturation" préparant la V0.6, note interne unique et historique compact de l'offre. Un état "Produit ou service introuvable" s'affiche proprement si le lien ne correspond à aucun élément du catalogue. Aucune action n'est encore modifiable (bouton "Modifier" en Work in progress, pas de modale) ;
