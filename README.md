...
> **Note de version — V0.1 à V0.4.1** : l'interface a fait l'objet d'une refonte graphique globale (V0.1), de corrections ciblées sur le tableau de bord (V0.1.1), d'une structuration technique Git/GitHub (V0.1.2), d'une réorganisation de la navigation principale (V0.2 : "Base de données" sortie de la sidebar, ajout de Trésorerie/Produits-Services/Finance), d'une connexion du tableau de bord à ces modules (V0.3) : les KPI, certaines alertes et certaines tâches sont de vrais liens vers les pages correspondantes, puis d'une première étape CRM sur la page Clients (V0.4.1, en cours sur la branche `v0.4.1-clients-crm-entry`) : recherche et filtre par statut fonctionnels, compteur dynamique, et accès à une première fiche client (`pages/fiche-client.html`). Voir `docs/changelog.md` pour le détail de chaque étape et `docs/versioning.md` pour la méthode de suivi Git recommandée.
...
- **Pages secondaires fictives** :
  ...
  - **Fiche client** *(nouveau, V0.4.1)* : première version générique de la fiche client (fil d'Ariane, retour vers la liste, carte d'identité factice), accessible depuis n'importe quelle ligne de la page Clients ; sera enrichie en V0.4.2 (historique, rendez-vous, devis, factures, notes, relances) ;
