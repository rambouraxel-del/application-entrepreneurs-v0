...
> **Note de version — V0.1 à V0.4.2** : l'interface a fait l'objet d'une refonte graphique globale (V0.1), de corrections ciblées sur le tableau de bord (V0.1.1), d'une structuration technique Git/GitHub (V0.1.2), d'une réorganisation de la navigation principale (V0.2 : "Base de données" sortie de la sidebar, ajout de Trésorerie/Produits-Services/Finance), d'une connexion du tableau de bord à ces modules (V0.3), d'une première étape CRM sur la page Clients (V0.4.1 : recherche et filtre par statut fonctionnels, compteur dynamique, accès à une fiche client), puis de la transformation de cette fiche en une vraie fiche CRM complète (V0.4.2, en cours sur la branche `v0.4.2-client-profile-crm`) : identité, KPI, informations, notes, historique des échanges, rendez-vous et documents cohérents selon le client sélectionné. Voir `docs/changelog.md` pour le détail de chaque étape et `docs/versioning.md` pour la méthode de suivi Git recommandée.
...
- **Pages secondaires fictives** :
  ...
  - **Fiche client** *(enrichie en V0.4.2)* : fiche CRM complète avec des données cohérentes selon le client sélectionné — en-tête (identité, statut, coordonnées, adresse, dates), 5 KPI (CA généré, relances en cours, rendez-vous à venir, documents récents, gestes commerciaux), bloc Informations client, bloc-notes commercial (3 dernières notes), historique des échanges (5 derniers événements), rendez-vous liés et documents & facturation (devis/factures/contrats). Un état "Client introuvable" s'affiche proprement si le lien ne correspond à aucun client ;
