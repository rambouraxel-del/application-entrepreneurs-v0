...
- **Pages secondaires fictives** :
  ...
  - **Facturation** *(liste de devis réelle depuis la V0.6.1)* : onglets "Devis" (actif) / "Factures" (à venir en V0.6.2), recherche et filtre par statut **réellement fonctionnels**, pagination, tableau de 5 devis fictifs couvrant les 4 statuts (Brouillon, Envoyé, Accepté, Refusé). Bouton "Créer un devis" réel ; "Créer une facture" reste en Work in progress ;
  - **Fiche devis** *(nouvelle, V0.6.1, `pages/devis-edition.html`)* : consultation, création, modification et duplication sur une seule page, sans assistant en plusieurs étapes. Client sélectionné dans la base Clients existante (référence vivante + informations figées par version au moment de la création), prestations ajoutées depuis le catalogue Produits/Services (uniquement celles au statut Actif) puis totalement indépendantes du catalogue une fois copiées dans une ligne. Lignes éditables (désignation, description, quantité, prix unitaire HT, taux de TVA, remise en %) avec calcul automatique HT/TVA/TTC en temps réel, arrondi à 2 décimales. Numérotation `DEV-AAAA-00001` à compteur annuel indicatif, versionnement (plusieurs versions d'un même devis, une seule active, historique consultable) et verrouillage automatique des devis Acceptés, démontrés sur un jeu de données fictif. Aucune action n'est encore enregistrée durablement (pop-up Work in progress sur Enregistrer/Supprimer/Créer une nouvelle version) ;
