...
> **Note de version — V0.1 à V0.4.3** : l'interface a fait l'objet d'une refonte graphique globale (V0.1), de corrections ciblées sur le tableau de bord (V0.1.1), d'une structuration technique Git/GitHub (V0.1.2), d'une réorganisation de la navigation principale (V0.2 : "Base de données" sortie de la sidebar, ajout de Trésorerie/Produits-Services/Finance), d'une connexion du tableau de bord à ces modules (V0.3), d'une première étape CRM sur la page Clients (V0.4.1 : recherche et filtre par statut fonctionnels, compteur dynamique, accès à une fiche client), de la transformation de cette fiche en une vraie fiche CRM complète (V0.4.2 + ajustements UX V0.4.2.1) : identité, KPI actionnables, informations, notes, historique des échanges, rendez-vous et documents cohérents selon le client sélectionné, puis de l'ajout d'interactions préparatoires sous forme de modales placeholder (V0.4.3, en cours sur la branche `v0.4.3-crm-placeholder-interactions`) : changement de statut, personnalisation des statuts, gestion des notes commerciales — aucune de ces actions ne modifie durablement les données. Voir `docs/changelog.md` pour le détail de chaque étape et `docs/versioning.md` pour la méthode de suivi Git recommandée.
