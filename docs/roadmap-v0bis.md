...
| V0.10.2 | Refonte du tableau de bord en cockpit quotidien (3 niveaux : Situation générale, Ma journée, Performance) | En attente de validation visuelle |
...
La phase V0.10 "Cohérence globale des parcours/Dashboard" annoncée dans les précisions précédentes est scindée en deux : **V0.10.1** traite la cohérence transversale UI/UX de l'ensemble des pages déjà construites (structure, composants communs, responsive minimal, documentation) sans toucher au tableau de bord au-delà de petites incohérences visuelles évidentes ; **V0.10.2** traite spécifiquement la refonte structurelle du tableau de bord, explicitement hors périmètre de la V0.10.1 pour ne pas mélanger un ajustement transversal léger avec une refonte de module plus lourde. Voir `docs/changelog.md` pour le détail des constats et corrections de la V0.10.1.

**V0.10.2** remplace l'empilement de blocs du tableau de bord accumulés depuis la V0.1 par une structure à trois niveaux (Situation générale, Ma journée, Performance), organisée autour de trois questions plutôt que d'un simple ajout successif de widgets : où en est mon entreprise, que dois-je faire aujourd'hui, comment évolue mon activité. Réutilise systématiquement les calculateurs déjà construits par les modules Facturation/Agenda/Trésorerie/Analyses (aucune formule monétaire ou métier réinventée), n'ajoute que les agrégations par période qui manquaient (CA/encaissé sur une période choisie, série mensuelle du graphique). Introduit `js/pilotage-config.js`, une configuration centralisée des préférences de pilotage (période par défaut, horizon de trésorerie, plage horaire de l'agenda), sans prétendre la faire persister entre les pages faute de backend. Voir `docs/changelog.md` et `docs/decisions.md` pour le détail des constats, des choix assumés (notamment l'exclusion du "Montant décaissé" du graphique, faute de profondeur historique fiable dans les données fictives) et des vérifications.
