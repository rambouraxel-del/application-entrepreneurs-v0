...
| **Lot 3 — Devis** | ✅ **Réalisé** : Devis, lignes, calculs en centimes, statuts, numérotation transactionnelle, immutabilité (trigger DB), PDF devis | Émettre un devis et télécharger son PDF — [`lot-3-devis.md`](lot-3-devis.md) |
| **Lot 4 — Factures et paiements** | ✅ **Réalisé** : identité légale Organization/Client, transformation devis accepté→facture (contrainte unique), gardien d'émission, numérotation (compteur séparé), immutabilité (trigger DB), Payment (partiel/idempotent/annulable, concurrence testée), statut de paiement dérivé, PDF facture, architecture e-invoicing (port + mapping, sans intégration réelle), Dashboard financier défini explicitement | Transformer un devis accepté en facture, l'émettre, l'encaisser — [`lot-4-factures-paiements.md`](lot-4-factures-paiements.md) |
...
| ADR-17 | `QuoteLine.quantity` en `Int` (millièmes entiers) au lieu du `Decimal(12,3)` prévu par `data-model.md` | **Révisée au Lot 3** — reste dans l'arithmétique entière (BigInt) validée au Lot 0, évite le type Prisma `Decimal` (sérialisation/comparaisons plus fragiles) | `lot-3-devis.md` |
| ADR-18 | Trigger d'immutabilité `quotes_protect_issued`/`quote_lines_protect_issued` implémenté au Lot 3 (premier document réellement "émis" de la V1) | Actée — priorité identifiée au Lot 0 (§7.3), réalisée à la première occasion utile | `lot-3-devis.md` |
| ADR-19 | Pas de colonne `amount_paid_cents` stockée sur `Invoice` : payé/restant dû **toujours dérivés** par somme des `Payment` actifs (non annulés) | **Révisée au Lot 4** — `data-model.md` prévoyait un agrégat maintenu dans la transaction de paiement ; la dérivation à la lecture élimine tout risque de désynchronisation, suffisante au volume MVP, réversible si le volume l'exige un jour | `lot-4-factures-paiements.md` |
| ADR-20 | `Invoice.status` reste `draft \| issued` uniquement ; le statut de paiement (impayée/partielle/payée/en retard) est un axe **orthogonal calculé à la volée**, jamais stocké | Actée — évite qu'un statut de paiement stocké devienne une troisième vérité en désaccord avec les paiements réels | `lot-4-factures-paiements.md` |
| ADR-21 | Numérotation facture sur un **compteur séparé** des devis (`document_counters`, `docType: 'invoice'`), même mécanisme transactionnel du Lot 0 sans modification | Actée | `lot-4-factures-paiements.md` |
| ADR-22 | Calculateur financier déplacé `modules/quotes/calc.ts` → `lib/billing/calc.ts`, partagé Devis **et** Facture | Actée — garantit une seule implémentation du calcul, jamais un fork | `lot-4-factures-paiements.md` |
| ADR-23 | `EInvoicePort`/`NullEInvoiceAdapter` implémentés au Lot 4 conformément à ADR-12 (§13.2) — architecture prête, aucun fournisseur choisi, aucun appel réseau | Actée — intégration réelle reportée au calendrier légal (P1) | `lot-4-factures-paiements.md` |
