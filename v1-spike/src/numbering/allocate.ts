import type { TenantScopedClient } from '../db/withTenant.js';

/**
 * Allocation d'un numéro de document.
 *
 * INTERDITS explicites (docs/v1/architecture.md §10) :
 *   - COUNT(*) + 1
 *   - MAX(number) + 1      ← c'est ce que fait la V0, non fiable en concurrence
 ...
 *   - la ligne du compteur est verrouillée pour la durée de la transaction, donc
 *     deux émissions concurrentes sont sérialisées : jamais le même numéro ;
 *   - l'incrément fait partie de la transaction d'émission : si celle-ci échoue,
 *     le compteur revient en arrière et le numéro est réutilisé par la suivante
 *     → séquence SANS TROU, ce qu'exige une numérotation de facture.
 */
export type CounterKey = { organizationId: string; docType: 'invoice' | 'quote'; year: number };
 ...
 *
 * La logique métier manipule (docType, year, sequence). Elle ne parse JAMAIS la
 * chaîne affichée : le format peut changer sans casser quoi que ce soit.
 */
